# Loop exercises - Nov 2018, rev Nov 2020
'''
Isabella Stone
...
def insertV1(L, i):
    ...
    # in^ L[0:4] is sorted
    # moves 3 left until its in place
...
def search(L, i, x):
    ...
    # whats the index of x where youd put it in 0,i
...
def insertV2(L, i):
    ...
    x = L[i] #value we want to insert
    insertAt = search(L, i, x) #the index it needs to be inserted at

    while L[insertAt] != x:
        for j in range(insertAt,i):
            swap(L, j, j+1)
...
def letterCounts(fname):
    ...
    #fill newDict w all letters and how many times they appear in file
    for line in file:
        for char in line:
            char = char.lower()
            if letter(char) == "non-letter":
                if "non-letter" in newDict:
                    newDict["non-letter"] += 1
                else: #add char to newDict if it's not already in it
                    newDict["non-letter"] = 1
