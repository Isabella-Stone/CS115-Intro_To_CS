...
def FA(x,y,cin):
    ...
    return (gor(gor(gand3(gnot(x), y, cin), gand3(x, gnot(y), cin)) , gor(gand3(x, y, gnot(cin)), gand3(x, y, cin))) ,
            gor(gor(gand3(gnot(x), gnot(y), cin), gand3(gnot(x), y, gnot(cin))) , gor(gand3(x, gnot(y), gnot(cin)), gand3(x, y, cin))))
...
def fourBitAdd(xxxx,yyyy):
    ...
    and c is the carry.'''
    (a,w) = FA(xxxx[3],yyyy[3],0)
    (b,x) = FA(xxxx[2],yyyy[2],a)
    (c,y) = FA(xxxx[1],yyyy[1],b)
    (d,z) = FA(xxxx[0],yyyy[0],c)
    return (d,(z, y, x, w))
...
def test_fourBitAdd():
    ...
    twelve =  (1,1,0,0)
    fifteen = (1,1,1,1)
    c,ww = fourBitAdd(five, ten)
    assert( ww == (1,1,1,1) and c == 0 )
    c,ww = fourBitAdd(ten, twelve)
    assert( ww == (0,1,1,0) and c == 1 )
    c,ww = fourBitAdd(twelve, fifteen)
    assert( ww == (1,0,1,1) and c == 1 )
    c,ww = fourBitAdd(five, fifteen)
